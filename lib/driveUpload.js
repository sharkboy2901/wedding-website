...
let _drive = null;
let _driveAccountEmail = null;

function getDriveClient() {
  const raw = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
  if (!raw) { _drive = null; _driveAccountEmail = null; return null; }

  try {
    const json = raw.trim().startsWith('{') ? raw : Buffer.from(raw, 'base64').toString('utf8');
    const creds = JSON.parse(json);

    // Re-use cached client only if it's the same service account
    if (_drive && creds.client_email === _driveAccountEmail) return _drive;

    _driveAccountEmail = creds.client_email;
    const auth = new google.auth.GoogleAuth({
      credentials: creds,
      scopes: ['https://www.googleapis.com/auth/drive'],
    });
    _drive = google.drive({ version: 'v3', auth });
    console.log('[Drive] Client initialised for:', _driveAccountEmail);
    return _drive;
  } catch (err) {
...
async function uploadToDrive(filePath, displayName, mimeType, folderId) {
  ...
  // Diagnostic: log exactly which service account and folder are being used
  try {
    const raw = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
    const json = raw.trim().startsWith('{') ? raw : Buffer.from(raw, 'base64').toString('utf8');
    const creds = JSON.parse(json);
    console.log('[Drive] Using service account:', creds.client_email, '| project:', creds.project_id);
  } catch (_) {}
  console.log('[Drive] Target folder ID:', folderId);
  console.log('[Drive] File:', filePath, '| mime:', mimeType);
  ...
  try {
    ...
    if (err.errors) console.error('[Drive] Error details:', JSON.stringify(err.errors));
    throw err;
  }
}
