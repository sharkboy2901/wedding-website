 ...
 *
 * 1. OAuth2 refresh token (works with personal Gmail / My Drive):
 *    GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET, GOOGLE_OAUTH_REFRESH_TOKEN
 *
 * 2. Service account JSON (works with Google Workspace Shared Drives only):
 *    GOOGLE_SERVICE_ACCOUNT_JSON  (raw JSON or base64-encoded)
 *    NOTE: Service accounts cannot upload to personal Google Drive (no storage quota).
 */
...
function getDriveClient() {
  // ── OAuth2 refresh token (personal Gmail / My Drive) ─────────────────────
  ...
  // ── Service account JSON (Google Workspace Shared Drives only) ────────────
  // NOTE: Service accounts cannot write to personal Google Drive (no storage quota).
  const raw = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
  if (!raw) { _drive = null; return null; }
